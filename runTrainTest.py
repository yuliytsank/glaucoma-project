...
from utils import get_data_info, custom3d_GAP_glaucoma
#import custom_transforms

# Training settings
extract_dir_default = os.path.expanduser(os.path.join('.', 'Images', 'Data', 'NP_Volumes', 'subsampled_all'))
...
parser.add_argument('--extract-dir', type=str, default=extract_dir_default, metavar='ED',
                    help='directory where training data is located (default: '+ '"'+extract_dir_default+'"')
parser.add_argument('--save-dir', type=str, default='models', metavar='SD',
                    help='directory to save models (default: "models")')

args = parser.parse_args()
args.cuda = not args.no_cuda and torch.cuda.is_available()

save_name = 'custom_3d_stats_all_glaucoma'
...
#            model.features = torch.nn.DataParallel(model.features)#only needed for multiple GPUs and makes loading/saving models more complicated
        ...
        train_loader = torch.utils.data.DataLoader(
                OCT_Folder(extract_dir, data_info = data_info_fold_train, transform = transform),
                batch_size=args.batch_size, shuffle=True, **kwargs)
        
        val_loader = torch.utils.data.DataLoader(
            OCT_Folder(extract_dir, data_info = data_info_fold_val),
            batch_size=args.batch_size, shuffle=True, **kwargs)
        test_loader = torch.utils.data.DataLoader(
            OCT_Folder(extract_dir, data_info = data_info_test),
            batch_size=args.batch_size, shuffle=True, **kwargs)
...
#                if (run_num==0)&(epoch == (epochs-1)):
                path_saved_model = os.path.join(save_dir, 'model_glaucoma_setSize'+ str(trainSet_size_num) +'_epoch_'+str(epoch)+'_run_' +str(run_num))
                print('Saving model...')
